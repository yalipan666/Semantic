...
PPath.FigPath = [rootdir 'Results' filesep 'ERF' filesep DS filesep]; %result path
...
if server == 1
    ...
    for mmm = 1:length(EpochType)
        ...
        %%% design matrix
        nsubj = 3;%length(ERF.subs);
        design = [1:nsubj 1:nsubj; ones(1,nsubj) ones(1,nsubj)*2];
        cfg.design = design;
        cfg.uvar   = 1;
        cfg.ivar   = 2;
        stat = ft_timelockstatistics(cfg, erf_1{6:8}, erf_2{6:8});
