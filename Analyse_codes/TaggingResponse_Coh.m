...
function TaggingResponse_Coh(sid)
...
if server == 1
    ...
    flickid = targid(eee);
    cfg = [];
    cfg.trials = find(epoch.trialinfo(:,tag_col) == flickid);
    data_1 = ft_selectdata(cfg, epoch);
    ...
    cfg.foi          = tagfreq;
    cfg.pad          = 'nextpow2';
    fourier          = ft_freqanalysis(cfg,data_1);%=% tfr.powspctrm:3D data: trialnum*channelnum*freqpoint
    ...
    for mmm = 1:length(EpochType)
        ...
        idx_2 = find(epoch.trialinfo(:,tag_col)==targid(mmm) & epoch.trialinfo(:,cond_col)==conds(1));
        idx_3 = find(epoch.trialinfo(:,tag_col)==targid(mmm) & epoch.trialinfo(:,cond_col)==conds(2));
        n_2 = length(idx_2);
        n_3 = length(idx_3);
        if n_2 > n_3
        ...
        cfg        = [];
        cfg.trials = idx_2;
        data_2   = ft_selectdata(cfg, epoch);
        cfg.trials = idx_3;
        data_3  = ft_selectdata(cfg, epoch);
        cfg        = [];
        cfg.trials = find(epoch.trialinfo(:,tag_col) == targid(mmm)); 
        data_1   = ft_selectdata(cfg, epoch);
        idx_1    = 1:size(data_1.trialinfo,1);
        
        %%% get trl numbers in each condition
        TrlNum = [size(data_1.trial,2) n_2 n_3];
        ...
        for ccc = 1:length(condnm)
            ...
            eval(['TagCoh.' EpochType{mmm} '_TrlIdx_equ{1,ccc} = idx_' num2str(ccc) ';']);
        end
    ...
    condtype = {'WrdFreq'};
    legend_all = [{[]},{'low'},{'high'}];
    HM_tim = zeros(length(sigsubid),length(EpochType),length(condnm)); %%% sub*epoch*condition
    PK_tim = HM_tim; %%% sub*epoch*condition
    HM_tim_grd = zeros(length(EpochType),length(condnm)); %%% epoch*condition
