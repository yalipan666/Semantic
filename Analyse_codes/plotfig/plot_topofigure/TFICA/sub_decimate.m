...
%   cutoff frequency 1*(Fs/2)/R, before resampling.
%
...
%   $Revision: 1.7.4.4 $  $Date: 2007/12/14 15:04:18 $
...
[errid,errmsg] = validateinput(idata,r);
...
if nargin == 2
    ...
    if ischar(nfilt)
        ...
        elseif nfilt(1) == 'i' || nfilt(1) == 'I'
            ...
            error(generatemsgid('InvalidEnum'),'Filter options are FIR and IIR.')
        end
        if nargin == 4
            nfilt = option;
        else
...
if fopt == 0	% FIR filter
    ...
    rip = .01;	% passband ripple in dB % <=====
    lp_f = 1; % lowpass cutoff frequency
    [b,a] = cheby1(nfilt, rip, lp_f/r);
    while all(b==0) || (abs(filtmag_db(b,a,lp_f/r)+rip)>1e-6)
        nfilt = nfilt - 1;
        if nfilt == 0
            break
        end
...
%--------------------------------------------------------------------------
function [errid, errmsg] = validateinput(x,r)
...
if (abs(r-fix(r)) > eps) || (r <= 0)
    errid = generatemsgid('invalidR');    
    errmsg = 'Resampling rate R must be a positive integer.';
    return;
end
