function result=EyelinkDoDriftCorrect_pan(el, x, y, draw, allowsetup)
...
if draw==1
	...
	targetrect=EyelinkDrawCalTarget_pan(el, x, y);     % we are told where it should be.
end
...
while result==el.NO_REPLY
    ...
    %%% YPan: remove key presses!
    if ~el.CheckEyeMyself
        [key, el]=EyelinkGetKey(el);		% getkey() HANDLE LOCAL KEY PRESS
    else
        WaitSecs(2);
        key = el.SPACE_BAR;
        el.allowlocaltrigger=1;
    end
