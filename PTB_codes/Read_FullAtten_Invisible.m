...
function Read_FullAtten_Invisible
tic
%%% tagging frequency
f1 = 60;
f2 = 60;
...
cfg.debugmode = 1; % runnning on local computer for rapidmode testing without triggers
...
cfg.TextSize = 10;  %22--0.32; %22-0.3556; 24--0.3951
cfg.TextColor = [0 0 0];
...
%%% get a full cycle of luminances changing for ending tagging smoothly
tag_one_cycle = freqTable(1,1:25);
tag_grey = [1 13 25];
...
for i = 1:nTrials
  ...
  %%% put all variables together in the sequence of {'tar','pos','tar_2','pos_2'};
  woff_mat = [woff_tar woff_pos woff_tar_2 woff_pos_2];
  mask_mat = [mask_tar mask_pos mask_tar_2 mask_pos_2];
  rect4offscreen_mat = [rect4offscreen_tar;rect4offscreen_pos;rect4offscreen_tar_2;rect4offscreen_pos_2];
  ...
  fix_X_pos_1 = [eye_fix_Xcoord(id_wrd(1)+1) eye_fix_Xcoord(id_wrd(1)+2)].*2;
  fix_X_tar_2 = [eye_fix_Xcoord(id_wrd(2)) eye_fix_Xcoord(id_wrd(2)+1)].*2;
  fix_X_pos_2 = [eye_fix_Xcoord(id_wrd(2)+1) eye_fix_Xcoord(id_wrd(2)+2)].*2;
  x_edges = [fix_X_tar_1 fix_X_pos_1(2) fix_X_tar_2 10000];%dividing x into 6 parts
  x_tag = [1 2 1 3 4 3];%should tag patch_id, in the sequence of {'tar','pos','tar_2','pos_2'};
  y_lim = ye.*2 + 30; % just add a random distance from y
  ...
  j = 1; %% index of the frames to change the frequency table in each frame
  fff = 1;
  tag_old = [0 0 0 0]; %variable to indicate whether the tagging location has changed
  tag_new = [0 0 0 0];
  while fff <= round(cfg.gaze_end/ifi) %% word frames during one trial    
    ...
    if cfg.el.eyelink
      ...
      if y < y_lim
        if sample.pa(eyeused_id)>0
            tmp = x_edges-x;
            tmp = find(tmp>=0);
            tag_new(x_tag(tmp(1))) = 1;
        end
      end
    else
      %%% randomly tag target or post-target if eye-tracker is off
      randchangeid = round(mod(GetSecs,6))
      if randchangeid == 0
        tag_new(1) = 1;
      elseif randchangeid == 1
        tag_new(2) = 1;
      elseif randchangeid == 2
        tag_new(1) = 1;
      elseif randchangeid == 3
        tag_new(3) = 1;
      elseif randchangeid == 4
        tag_new(4) = 1;
      else
        tag_new(3) = 1;
      end
    ...
    if any(tag_old ~= tag_new) %fixate to another word
       % get the colormat to end the old tagging word at grey
       end_tag = dsearchn(tag_one_cycle',cur_freqTable(1,(((j-1)*12)+12)));
       end_grey = tag_grey(tag_grey-end_tag>0);
       end_color = tag_one_cycle(end_tag+1:end_grey(1));
       j
       end_color = [end_color 0.5.*ones(1,12-length(end_color))]
       cur_freqTable(1,1)
       % start the new tagging word from grey
       j = 1;
       old_id = find(tag_old);
       tag_old = tag_new;
    end
    ...
    for q = 1:4 %for all quadrants
      %%% end-tagging for the previous tagging word to make sure it stops at grey
      if any(tag_old ~= tag_new)
          tag_color = [end_color(1,q),end_color(1,(q+4)),end_color(1,(q+8))];
          Screen('DrawTexture',window,woff_mat(old_id),[],q_rects(q,:),0,[],1,tag_color);
          Screen('DrawTexture',window,mask_mat(old_id),[],rect4offscreen_mat(old_id,:)+[q_rects(q,1) q_rects(q,2) q_rects(q,1) q_rects(q,2)]);
      end
      
      %%% tagging the new word, or keeping tagging the same word if the
      %%% fixaiton doesn't move to another word
      tag_color = [cur_freqTable(1,(((j-1)*12)+q)), cur_freqTable(1,(((j-1)*12)+q+4)),  cur_freqTable(1,(((j-1)*12)+q+8))];
      new_id = find(tag_new)
      Screen('DrawTexture',window,woff_mat(new_id),[],q_rects(q,:),0,[],1,tag_color);
      Screen('DrawTexture',window,mask_mat(new_id),[],rect4offscreen_mat(new_id,:)+[q_rects(q,1) q_rects(q,2) q_rects(q,1) q_rects(q,2)]);
